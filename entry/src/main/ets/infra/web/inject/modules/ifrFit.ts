...
window.__safeInject('iframeFit', function () {
  ...
  var SCALE_ID       = 'kc-scale';  // 内层真正缩放的元素（基准 1200×720）
  var policy = 'auto';
  var lastPolicy = 'auto';
  var TOP_GAP = 0;
  var LOCK = false;
  ...
  var TARGET_WRAP  = null; // #kc-host
  var TARGET_SCALE = null; // #kc-scale（真正进行 transform 的节点）
  ...
  function findGameHost() {
    ...
    for (var j = 0; j < cands.length; j++) { var e = document.querySelector(cands[j]); if (e) return { el: e, mode: 'div' }; }
    var cv = document.querySelector('canvas');
    if (cv) { var p = cv.parentNode && cv.parentNode.nodeType === 1 ? cv.parentNode : cv; return { el: p, mode: 'div' }; }
    return null;
  }
  ...
  function ensureTargets(hostEl, mode) {
    ...
    // 给缩放目标打一个固定 ID，便于 CSS 选择器控制
    if (scaleEl.id !== SCALE_ID) {
      try { scaleEl.id = SCALE_ID; } catch(_){}
    }
  ...
  // 4) 基础样式：iframe 固定 1200×720；缩放目标 #kc-scale 也固定 1200×720
  function applyAlignCSS() {
    ...
    s.textContent =
      ...
      '#'+SCALE_ID+'{position:absolute!important;top:0!important;left:0!important;width:'+W+'px!important;height:'+H+'px!important;border:0!important;background:#000!important;display:block!important;backface-visibility:hidden!important}'+
  ...
  // 5) 把 transform 仅打在 #kc-scale 上
  function writeFitCSS(scale, off) {
    var s = ensureStyle(STYLE_FIT_ID);
    var ox = snap(off.x), oy = snap(off.y);
    s.textContent =
      '#'+SCALE_ID+'{transform-origin:0 0!important;transform:translate3d('+ox+'px,'+oy+'px,0) scale('+scale+') !important;will-change:transform!important}';
  }
